...
def show_user_info(user_id):
    user_detail = get_info_by_user_id(user_id, platform='X')
    st.markdown("---")
...
def delete_user(user_id):
    st.write("Xóa người dùng X khỏi cơ sở dữ liệu")
    if user_id:
        st.write(f"Bạn có chắc chắn muốn xóa người dùng {user_id} không?")
        if st.button("Xóa"):
            delete_user_by_id(user_id, platform='X')
...
def display_x_users():
    ...
    user_ids = get_distinct_user_ids_from_db(platform='X')
    ...
    if user_ids:
        for idx, user_id in enumerate(user_ids, start=1):
            ...
            if user_id is not None:
                ...
                user_link = f"[https://www.x.com/@{user_id}](https://www.x.com/@{user_id})"
...
def add_x_user():
    ...
    def check_task_status():
        if task_status_list[0]["Trạng thái"] == "thành công" and task_status_list[1]["Trạng thái"] == "thành công" and task_status_list[2]["Trạng thái"] == "thành công":
            return True
        return False
