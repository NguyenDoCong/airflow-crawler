...
def show_user_info(user_id):
    user_detail = get_info_by_user_id(user_id, platform='instagram')
    st.markdown("---")
...
def delete_user(user_id):
    st.write("Xóa người dùng instagram khỏi cơ sở dữ liệu")
    if user_id:
        st.write(f"Bạn có chắc chắn muốn xóa người dùng {user_id} không?")
        if st.button("Xóa"):
            delete_user_by_id(user_id, platform='instagram')
...
def display_instagram_users():
    ...
    user_ids = get_distinct_user_ids_from_db(platform='instagram')
    ...
    if user_ids:
        for idx, user_id in enumerate(user_ids, start=1):
            ...
            if user_id is not None:
                ...
                user_link = f"[https://www.instagram.com/{user_id}](https://www.instagram.com/{user_id})"
